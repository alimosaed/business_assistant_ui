...
const useSocket = (
  ...
  const connectWs = useCallback(() => {
    ...
    const wsURL = new URL(`${process.env.NEXT_PUBLIC_WS_URL}/connect`);
    
    // Get the JWT token from localStorage
    const token = localStorage.getItem('authToken');
    
    // Include the token in the WebSocket URL if available
    if (token) {
    ...
    const timeoutId = setTimeout(() => {
      if (newWs.readyState !== 1) {
        toast.error(
          'Failed to connect to local server. Please ensure server is running on port 5001.'
        );
      }
    }, 10000);
    ...
    newWs.addEventListener('message', (e) => {
      ...
      if (data.type === 'signal' && data.data === 'open') {
        // After receiving the open signal, send auth message with token as fallback
        const token = localStorage.getItem('authToken');
        if (token) {
        ...
        console.log('[DEBUG] Connected to local server');
      }
      if (data.type === 'auth_success') {
        console.log('[DEBUG] Authentication successful:', data.data);
      }
      if (data.type === 'auth_error') {
        console.error('[DEBUG] Authentication failed:', data.data);
    ...
    newWs.onerror = () => {
      clearTimeout(timeoutId);
      setError(true);
      toast.error('WebSocket connection error - Is local server running?');
    };
    
    newWs.onclose = () => {
      clearTimeout(timeoutId);
      setError(true);
      console.log('[DEBUG] Connection to local server closed');
    };
    
    setWs(newWs);
  }, [setIsWSReady, setError]);
